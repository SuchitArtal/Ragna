...
class Agent:
    ...
    def propose_edit(self, file_path: str, modification_description: str, patch_text: str, confidence: float = 0.75) -> Dict[str, Any]:
        ...
        # Ensure any hunk header lines are valid unified-diff headers.
        # Some internal helpers produce a minimal '@@' marker; make it
        # a syntactically valid hunk header so downstream parsers/appliers
        # won't fail.
        lines = full_patch.splitlines()
        for idx, ln in enumerate(lines):
            if ln.startswith("@@"):
                # If header doesn't contain numeric ranges, replace with a safe default
